...
| HTTPS HTTP/3 negotiation | `unsupported-by-policy` | No promotion evidence is required. A future support claim needs a superseding ADR plus trusted x64 WinHTTP/QUIC, focused integration, release build, and consumer-smoke evidence |
...
prove negotiated HTTP/2 support. The recorded HTTP/2 result is host-specific.
...
before that host-specific row is promoted; no HTTP/3 record is accepted under
