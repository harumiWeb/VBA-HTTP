...
try {
    ...
    $tests = @($result.tests)
    if ($result.status -ne "ok" -or $tests.Count -ne 55 -or @($tests | Where-Object status -ne "passed").Count -ne 0) {
        throw "Integration suite did not return fifty-five passing tests."
    }
}
finally {
