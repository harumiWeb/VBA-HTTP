...
try {
    ...
    $tests = @($result.tests)
    if ($result.status -ne "ok" -or $tests.Count -ne 51 -or @($tests | Where-Object status -ne "passed").Count -ne 0) {
        throw "Integration suite did not return fifty-one passing tests."
    }
}
finally {
